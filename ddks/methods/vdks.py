...
class vdKS(ddKS):
    ...
    def calcD(self, pred, true):
        ...
        if self.approx:
            ...
            for v_id in self.voxel_list.keys():
                ...
                #print(f'ts/trueshape {len(ts)/self.true.shape[0]} ps/predshape {len(ps)/self.pred.shape[0]}')

                tmp_diff = len(ts)/self.true.shape[0]*os_tp-os_pp*len(ps)/self.pred.shape[0]
                #print(torch.max(torch.abs(tmp_diff)))
    ...
    def permute(self, J=1_000):
        ...
        for j in range(J):
            ...
            _d2 = all_pts[idx2]
            T_[j] = self(_d1, _d2)
        return float(torch.sum(T_ > T) +1)/ float(J+2), T, T_
